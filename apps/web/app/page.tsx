...
export default function Home() {
  ...
  useEffect(() => {
    const onHash = () => { setView(currentView()); void refresh(); };
    const timer = window.setTimeout(onHash, 0);
    const interval = window.setInterval(() => { void refresh(); }, 20_000);
    window.addEventListener('hashchange', onHash);
    return () => { window.clearTimeout(timer); window.clearInterval(interval); window.removeEventListener('hashchange', onHash); };
  }, [refresh]);

  function navigate(next: View) { if (window.location.hash === `#${next}`) { setView(next); void refresh(); } else window.location.hash = next; }
...
function Overview({ data, navigate }: { data: WorkspaceData; navigate: (view: View) => void }) {
  ...
  return <><section className="attention-grid" aria-label="Compliance summary">
    ...
    <section className="panel readiness-panel"><PanelHeading title="Facility readiness" action="View all facilities" onAction={() => navigate('facilities')} /><div className="readiness-table">{data.facilities.map(facility => <div className="readiness-row" key={facility.id}><span className="facility-row-icon" aria-hidden="true">{initials(facility.name)}</span><div><strong>{facility.name}</strong><small>{facility.location}</small></div><div className="progress"><span style={{ width: `${facility.readiness}%` }} /></div><strong>{facility.readiness}%</strong><span className={`risk-pill ${facility.risk.toLowerCase()}`}>{facility.risk === 'HIGH' ? 'Attention' : 'On track'}</span></div>)}</div></section>
    <section className="panel review-queue"><PanelHeading title="Review queue" /><div className="queue-grid"><button type="button" onClick={() => navigate('field')}><i aria-hidden="true">▣</i><strong>{pendingEvidence}</strong><span>Field submissions</span><small>Awaiting decision</small></button><button type="button" onClick={() => navigate('documents')}><i aria-hidden="true">◈</i><strong>{data.dashboard.pendingReviews}</strong><span>Approval proposals</span><small>Awaiting decision</small></button><button type="button" onClick={() => navigate('obligations')}><i aria-hidden="true">▤</i><strong>{data.obligations.filter(item => item.status === 'AWAITING_REVIEW').length}</strong><span>Obligations</span><small>Evidence received</small></button></div></section>
  </div></>;
}
...
function FacilitiesView({ data, navigate }: { data: WorkspaceData; navigate: (view: View) => void }) {
  return <section className="panel facilities-page"><PanelHeading title={`${data.facilities.length} operating sites`} action="Open obligation register" onAction={() => navigate('obligations')} /><div className="facility-cards">{data.facilities.map(facility => {
    ...
    return <article className={`facility-card ${health}`} key={facility.id}><div className="facility-card-header"><div className="facility-card-top"><span className="facility-icon">{initials(facility.name)}</span><span className={`facility-health ${health}`}>{healthLabel}<i /></span></div><h2>{facility.name}</h2><p>{facility.client} · {facility.location}</p></div><div className="facility-card-body"><span className="technical-label">Readiness score</span><div className="facility-score"><strong>{facility.readiness}%</strong><div className="progress"><span style={{ width: `${facility.readiness}%` }} /></div></div><dl><div><dt>Open obligations</dt><dd>{open}</dd></div><div><dt>High risk</dt><dd>{high}</dd></div></dl><div className="insight"><strong>Why this score</strong><span>{explanation}</span></div></div></article>;
  })}</div></section>;
}

function ObligationsView({ data, refresh, notify, navigate, openSubmission }: { data: WorkspaceData; refresh: () => Promise<void>; notify: (message: string) => void; navigate: (view: View) => void; openSubmission: (id: string) => void }) {
...
function DocumentsView({ data, refresh, notify, importRequest, onImportRequestHandled }: { data: WorkspaceData; refresh: () => Promise<void>; notify: (message: string) => void; importRequest: number; onImportRequestHandled: () => void }) {
  ...
  const reviewQueue = pendingProposals.length ? pendingProposals : data.proposals;
  const selected = reviewQueue.find(item => item.id === selectedId) ?? reviewQueue[0];
  const document = data.documents.find(item => item.id === selected?.documentId) ?? data.documents[0];
  const position = Math.max(reviewQueue.findIndex(item => item.id === selected?.id), 0);
  const remaining = pendingProposals.length;
  useEffect(() => {
  ...
  function move(offset: number) { if (!reviewQueue.length) return; const next = reviewQueue[(position + offset + reviewQueue.length) % reviewQueue.length]; setSelectedId(next.id); setEditing(false); }
  return <><div className="document-toolbar"><div className="document-selector"><label>Import for<select aria-label="Facility for imported approval" value={facilityId} onChange={event => setFacilityId(event.target.value)}>{data.facilities.map(item => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><input ref={fileRef} hidden tabIndex={-1} type="file" accept="application/pdf,.pdf" onChange={event => void importPdf(event.target.files?.[0])} /><button type="button" className="secondary-button upload-button" onClick={() => fileRef.current?.click()} disabled={Boolean(busy)}>{busy === 'Importing approval…' ? busy : 'Upload approval PDF'}</button></div><div className="proposal-progress"><strong>{remaining} proposal{remaining === 1 ? '' : 's'} {remaining === 1 ? 'needs' : 'need'} a decision</strong><span><button type="button" aria-label="Previous proposal" onClick={() => move(-1)}>‹</button>Proposal {reviewQueue.length ? position + 1 : 0} of {reviewQueue.length}<button type="button" aria-label="Next proposal" onClick={() => move(1)}>›</button></span></div></div>
...
function FieldView({ data, refresh, notify, initialSubmissionId }: { data: WorkspaceData; refresh: () => Promise<void>; notify: (message: string) => void; initialSubmissionId: string | null }) {
  const orderedSubmissions = [...data.submissions].sort((left, right) => new Date(right.completedAt).getTime() - new Date(left.completedAt).getTime());
  const defaultSubmission = orderedSubmissions.find(item => item.reviewStatus === 'PENDING') ?? orderedSubmissions[0];
  const [selectedId, setSelectedId] = useState(initialSubmissionId ?? defaultSubmission?.id ?? ''); const [note, setNote] = useState(''); const [busy, setBusy] = useState(false); const [previewPhoto, setPreviewPhoto] = useState<number | null>(null); const selected = data.submissions.find(item => item.id === selectedId) ?? defaultSubmission; const events = selected ? data.auditEvents.filter(item => item.entityType === 'FieldSubmission' && item.entityId === selected.id) : []; const gps = selected?.notes.match(/GPS:\s*(-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)/); const fieldNote = selected?.notes.split(' · ')[0] ?? '';
  ...
  return <div className="field-review-layout"><aside className="submission-list"><h2>Submissions <small>Select to review</small></h2>{orderedSubmissions.map(item => <button type="button" className={item.id === selected.id ? 'active' : ''} aria-pressed={item.id === selected.id} key={item.id} onClick={() => setSelectedId(item.id)}><span className={`review-dot ${item.reviewStatus.toLowerCase()}`} /><div><strong>{item.obligation.title}</strong><small>{item.inspector} · {formatDateTime(item.completedAt)}</small></div><div className="submission-record-footer"><StatusPill status={item.reviewStatus} /><span className="record-action">{item.id === selected.id ? 'CURRENT' : 'OPEN REVIEW →'}</span></div></button>)}</aside>
...
function PhotoLightbox({ index, count = evidencePhotos.length, onClose }: { index: number; count?: number; onClose: () => void }) {
  const photos = evidencePhotos.slice(0, Math.min(count, evidencePhotos.length));
  ...
  function move(offset: number) { setActiveIndex(value => (value + offset + photos.length) % photos.length); }
...
function formatDue(value: string) { const today = new Date(); today.setHours(0, 0, 0, 0); const days = Math.round((new Date(`${value}T00:00:00`).getTime() - today.getTime()) / 86400000); return days < 0 ? `Overdue by ${Math.abs(days)} day${Math.abs(days) === 1 ? '' : 's'}` : days === 0 ? 'Due today' : days === 1 ? 'Due tomorrow' : `Due in ${days} days`; }
