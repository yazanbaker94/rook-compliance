...
function FieldView({ data, refresh, notify, initialSubmissionId }: { data: WorkspaceData; refresh: () => Promise<void>; notify: (message: string) => void; initialSubmissionId: string | null }) {
  const featuredSubmission = data.submissions.reduce<Submission | undefined>((best, item) => !best || item.photoCount > best.photoCount ? item : best, undefined);
  const [selectedId, setSelectedId] = useState(initialSubmissionId ?? featuredSubmission?.id ?? ''); const [note, setNote] = useState(''); const [busy, setBusy] = useState(false); const [previewPhoto, setPreviewPhoto] = useState<number | null>(null); const selected = data.submissions.find(item => item.id === selectedId) ?? featuredSubmission; const events = selected ? data.auditEvents.filter(item => item.entityType === 'FieldSubmission' && item.entityId === selected.id) : []; const gps = selected?.notes.match(/GPS:\s*(-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)/); const fieldNote = selected?.notes.split(' · ')[0] ?? '';
